...
%closed-loop
[Tx, X_x, U_x] = rocket.simulate_f(sys_x, [0;0;0;0], Tf, @mpc_x.get_u, ref_x);
...
%closed-loop
[Ty, X_y, U_y] = rocket.simulate_f(sys_y, [0;0;0;0], Tf, @mpc_y.get_u, ref_y);
...
U_optz = U_optz + us([3]); % u=[delta1 delta2 Pavg Pdiff]
ph = rocket.plotvis_sub(T_optz, X_optz, U_optz, sys_z, xs, us);
title("Z open-loop");

%closed-loop
[Tz, X_z, U_z] = rocket.simulate_f(sys_z, [0;0], Tf, @mpc_z.get_u, ref_z);
...
%closed-loop
[Tr, X_r, U_r] = rocket.simulate_f(sys_roll, [0;0], Tf, @mpc_roll.get_u, ref_roll);
